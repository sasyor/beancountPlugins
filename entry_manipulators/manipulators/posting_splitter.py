...
class DiscountSplitter:
    ...
    def split(self, entry):
        ...
        return self.__replace_accounts(new_entry)
    ...
    def __replace_accounts(self, entry):
        ...
        for posting in entry.postings:
            ...
            if posting.account not in account_consolidators:
                account_consolidators[posting.account] = AccountConsolidationData(posting.account,
                                                                                  posting.account + ":Price",
                                                                                  {posting.account + ":Discount"})
...
class PostingSplitter(EntryManipulator):
    ...
    def execute(self, entry: data.Transaction) -> EntryManipulationResultData:
        new_entry, account_consolidators = self.__split_single_entry(entry)

        return EntryManipulationResultData([new_entry], account_consolidators)

    def __split_single_entry(self, entry):
        if entry.meta and self.metadata_name_type in entry.meta:
            return self.__get_entry_level_splitter().split(entry)  # TODO extract to new manipulator
